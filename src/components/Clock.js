...
function Clock() {
  ...
  const updateTasks = () =>  {
    ...
    //tasks.forEach(task => {
      console.log('started for each', currentHour, currentMinutes)
      if (currentDay === 1) {
        console.log('today is monday')
        if (currentHour === 16 && currentMinutes === 38) {
        ...
        } if (currentHour === 16 && currentMinutes === 7) {
        ...
        console.log('thusday')
        if(currentHour === 13 && currentMinutes === 53){
          newDisplayTasks.push({task: 'this works', interns: getRandomInterns(1)},
          {task: 'test2', interns: getRandomInterns(2)});
        }
      }
      setDisplayTasks(newDisplayTasks);
      console.log('new display task set')
    //});
  }
  //ssh key
  useEffect(() => {
    ...
    if (currentDateTime.getSeconds() === 0) {
      updateTasks();
      console.log('updateTasks set off')
    }
    // eslint-disable-next-line
  }, [currentDateTime]);
  ...
  const handleCheckboxCheck = (event, taskIndex) => {
    // Access the checked state of the checkbox from the event object
    const isChecked = event.target.checked;
    // Now you can use isChecked in your logic
    if (isChecked) {
      ...
      console.log(`Task ${taskIndex} is unchecked.`);
